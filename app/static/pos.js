  ...
  tbody.innerHTML = rows.map(p => {
    ...
    return `<tr${detParts.length ? ' style="border-bottom:none"' : ''}>
      ...
      <td class="pend-actions">
        ...
        ${ec === 'pendiente_pago' && p.cliente_telefono ? `<button class="btn-fin" style="background:#d4a843;color:#193a2c" onclick="aprobarEnviarTicket(${p.id})">📨 ${p.ticket_enviado_at ? 'Reenviar' : 'Enviar'} ticket</button>` : ''}
